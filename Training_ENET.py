...
model = ElasticNet(alpha=0.5, l1_ratio=0.5)  # You can adjust alpha and l1_ratio as needed

# Train the Elastic Net model on the scaled training set
...
print(f'Elastic Net Regression Mean Squared Error: {mse:.2f}')

EndTime = datetime.today()

with open(output_file, 'a', newline='') as csvfile:
    csv_writer = csv.writer(csvfile)
    data_row = [StartTime.strftime("%Y-%m-%d_%H:%M:%S"),EndTime.strftime("%Y-%m-%d_%H:%M:%S"),"ENET",mse]
