...
# Train the Bayesian Ridge model on the scaled training set
...
# Evaluate the Bayesian Ridge model using mean squared error
mse = mean_squared_error(y_test, y_pred_bayesian)
print(f'Bayesian Ridge Regression Mean Squared Error: {mse:.2f}')

EndTime = datetime.today()

with open(output_file, 'a', newline='') as csvfile:
    csv_writer = csv.writer(csvfile)
    data_row = [StartTime.strftime("%Y-%m-%d_%H:%M:%S"),EndTime.strftime("%Y-%m-%d_%H:%M:%S"),"Bayesian",mse]
